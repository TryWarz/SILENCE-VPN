	...
	if [[ $OS == 'arch' || $OS == 'fedora' || $OS == 'centos' || $OS == 'oracle' ]]; then
		...
		function newClient() {
			...
			if [[ $CLIENTEXISTS == '1' ]]; then
				echo ""
				echo "The specified client CN already exists in easy-rsa. Please choose another name."
				exit
			else
				cd /etc/openvpn/easy-rsa/ || return
				case $PASS in
...
# for execute function exemple : ./openvpn-install.sh -install
...
if [[ $1 == "-install" ]]; then
	...
	echo "Usage:"
	echo "  Command: ./openvpn-install.sh -install"
	echo "  Command: ./openvpn-install.sh -client"
	echo "  Command: ./openvpn-install.sh -revoke"
	echo "  Command: ./openvpn-install.sh -remove"
	echo "  Command: ./openvpn-install.sh -unbound"
fi
